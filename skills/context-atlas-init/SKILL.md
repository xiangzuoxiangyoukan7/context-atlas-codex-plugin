...
Build an initialization Proposal that conforms to `../../assets/schemas/initialization-proposal.schema.json`. Compute `proposal_revision` from canonical JSON excluding that field, display the same revision with the human-readable Proposal, and require confirmation of that exact revision. Do not ask the user to write JSON or provide low-level file parameters. Repository observations prove implementation facts only: keep product meaning, design reasons, approval, and anything not discoverable as `proposed` or in `unknowns`.
