...
2. `initialization-*.schema.json` 和 `ingest-report.schema.json` 使用标准 JSON Schema 约束会话 Proposal 或报告；`embedded-source.schema.json` 定义格式 4 及后续格式中知识项就地携带的来源对象。这些都不登记进知识类型 `catalog.json`。
